...
### Strip assignment
Tissue strips (control / infected / control) identified by fitting a Gaussian Mixture Model (GMM, 3 components) to the **PCA-rotated** x-coordinate (`x_rot_px`) within each FOV (`reassign_strips_gmm`; rotation from `add_rotated_coords`, both in `00_functions.ipynb`; applied via nb02c / nb10). Per-FOV decisions recorded in the `fov_config` dictionary for reproducibility. Output: `s1_all_strips.parquet`.

### Rogue transcript removal (DBSCAN)
Transcripts far outside the tissue boundary inflate the observation window and bias the K-function. Removed using DBSCAN (density-based spatial clustering). `eps` derived adaptively from the 97th percentile of 1-nearest-neighbour distances (sklearn `NearestNeighbors`), clipped to [20, 30] px; `min_samples = 5`. In the canonical run (nb10, grouped by FOV on `x_rot_px`), **7.6%** of 702,873 transcripts are flagged as noise (method developed in `08_improved_QC.ipynb`), retained with an `is_noise` flag. A further 2.0% small-cluster + 5.4% manual exclusion (`apply_cleanup`, 18 hand-picked clusters) leaves 611,150 (87%) for analysis.
