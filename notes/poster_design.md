...
*Caption.* "One field of view, three layers. NoSeggs discards the segmentation
...
capability table (capitalised headers, NoSeggs row highlighted green, alternating
...
NoSeggs (all four ✓), CellChat/LIANA, Squidpy gr.ripley, Bento, Baysor, FICTURE,
spatstat (R). NoSeggs is the only all-✓ row.
...
NoSeggs satisfies all four capabilities."

**M6 — Workflow flowchart + quick-start guide.**
File: `results/figures/14_M6_workflow.png`. Generated by nb14 cell `m6-flow-code`.
Portrait flowchart, 8 stages top→bottom: INPUT (raw transcripts) → 1 PCA align +
per-FOV GMM strips → 2 DBSCAN noise removal → 3 observation window → 4 bivariate
Ripley K → L(r) → 5 label-swap permutation envelope → 6 effect-size (SES) ranking
→ OUTPUTS (networks · UMAP · heatmaps). Each stage carries an amber option-chip
naming its tunable choice (GMM 3-component; adaptive ε = p97 1-NN [20,30] px;
rect/convex/concave window; multi-scale r-grid; n_sim 99/199 + edge correction;
binary vs continuous SES). Purple quick-start panel at the foot: "use NoSeggs
when… segmentation fails / want L–R co-localisation from transcripts / imaging
platforms (CosMx, MERFISH, Xenium) / ≥ ~50 transcripts per gene per ROI."
*Caption.* "The end-to-end workflow and a quick-start guide; amber chips mark the
tunable options at each step."

### Layout recommendation (A0 portrait, three columns) — REVISED 2026-06-04

> **BUILT 2026-06-04 evening → `docs/NoSeggs_Poster_A0.pptx`** (true A0, native
> editable PowerPoint; original A1 `NoSeggs_Poster.pptx` kept as fallback). The
> layout below is realised in that file with `[ FILL ]` guide prompts + per-figure
> legends. Regenerate with `docs/_build_poster.py`. Remaining: Harry fills prose +
> fact-checks; optional Arial re-run of nb13 results figs for font consistency.
