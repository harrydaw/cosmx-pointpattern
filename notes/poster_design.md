...
> **Print deadline: Saturday 6 June 2026.** Travel Rome Sunday 7. A0 portrait. Working file: `docs/NoSegs_Poster.pptx`. Audience: computer scientists + neuroscientists.
...
**"NoSegs: Segmentation-Free Ligand–Receptor Co-Localisation in Spatial Transcriptomics"**
...
>
> NoSegs takes a different route: treat each gene as a 2D point pattern in tissue space, and use bivariate Ripley's K-functions with label-permutation envelopes to detect ligand–receptor co-localisation directly. No cells. No clusters. Just transcripts.
...
> **Negative-2 row dropped 2026-06-04.** KRT8 × SCGB3A1 was never run at panel
> scale (absent from both `lr_panel_results*.parquet`), so it was a fabricated
> placeholder. Removed rather than invented. The Positive vs Negative pair already
> carries the +2.67-half-width discrimination point. If a third control is wanted
> for the dissertation, recompute KRT8 × SCGB3A1 live via nb13 §7 (`peak_signed_ses`).
...
## Methodology emphasis (NEW direction 2026-06-04)

Decision: dedicate a substantial chunk of the poster to **methodology + tool
comparison**, not just results. Rationale (Harry, 2026-06-04): the bulk of the
notebook work (00–09) was data-wrangling that turns an *unanalysable* dataset
into an analysable one; foregrounding it (a) is genuinely the contribution, and
(b) keeps the poster defensible without leaning hard on the bolder results
claims. Storyline: *"I got data → vendor segmentation failed → attempts to fix
it failed → no existing pipeline lets me avoid binning 100 GB of data that cost
tens of thousands of pounds → here are the tools I built to analyse it anyway."*

A **suite of separate, individually polished 300-dpi panels** carries this
(REVISED 2026-06-04: replaces the earlier single composite Fig M + matrix Fig T).
All generated by **`notebooks/14_poster_methods.ipynb`** (reuses nb00 helpers and
the canonical `s1_all_strips_cleaned.parquet`; every stat read live from data).
The standalone `scripts/poster_fig_*.py` and the old `13_M`/`13_T` PNGs are
**superseded and deleted**.

**M0 — Raw data, three layers (the grounding view).**
File: `results/figures/14_M0_raw_data.png`. Generated by nb14 cell `m0-raw-code`,
read live from `data/raw/Varsha_1234.zarr` (FOV 4).
1×3: (a) raw DAPI morphology; (b) vendor cell-segmentation mask (148 cells, the
discarded layer); (c) raw transcripts coloured assigned (blue) vs unassigned
`cell_ID==0` (magenta, ~40 % of points).
*Caption.* "One field of view, three layers. NoSegs discards the segmentation
mask and carries every transcript through — including the ~40 % that segmentation
never assigns."

**M1 — Vendor segmentation dropout (the grounding figure).**
File: `results/figures/14_M1_segmentation_dropout.png`.
2-panel: left = Assigned (56.4 %) vs Unassigned (**43.6 %**, cell_ID = 0) bar with
counts + %; right = per-FOV % unassigned with dataset-mean line (consistent ~38–55 %).
*Caption.* "Vendor segmentation never assigns 43.6 % of transcripts to a cell, so
every cell-level tool silently discards them — consistently across all 12 FOVs."

**M2 — GMM strip assignment.**
File: `results/figures/14_M2_gmm_strips.png`.
Left = representative FOV (4) rotated-coords scatter, three crisp vertical strips
coloured by assignment with GMM component-midpoint boundaries; right = rotated-x
histograms for FOVs 4/5/8, trimodal, with the same boundaries.
*Caption.* "After PCA aligns the tissue axis, a per-FOV 3-component Gaussian
mixture separates the three tissue strips; boundaries are set per FOV, with manual
overrides where the mixture is ambiguous."

**M3 — Adaptive-DBSCAN noise QC.**
File: `results/figures/14_M3_dbscan_qc.png`.
Left = FOV-4 scatter (kept / small-cluster / noise); right = whole-dataset
retention bar, **87 %** kept (7.6 % noise + 2.0 % small + 5.4 % manual). ε = p97
1-NN, clipped [20, 30] px (annotated).
*Caption.* "DBSCAN with an adaptively-set ε flags diffuse background and stray
clusters; 87 % of transcripts survive QC."

**M4 — Observation-window iterations.**
File: `results/figures/14_M4_window_iterations.png`.
Per-strip overlay rect bbox vs convex hull vs concave hull (ratio 0.1) with areas
in Mpx² — concave hugs the thin curved strips far tighter (e.g. strip 2: rect 313
→ convex 61 → concave 14 Mpx²).
*Caption.* "The observation window is the dominant lever for inferential power. A
concave hull tracks the true tissue extent, shrinking the area ~20× vs the
bounding box and tightening the permutation null."

**M5 — R-value / scale design + edge correction.**
File: `results/figures/14_M5_rscale.png`.
1×3: (a) two-segment r-spacing schematic (dense cell–cell [5,50] px ×30, coarse
tissue (50,250] px ×20); (b) concentric rings at 4 r on a real KRT8 transcript;
(c) edge-correction disc clipped at the hull boundary with the inside-hull weight.
*Caption.* "A deliberate multi-scale r-grid resolves both cell-scale and
tissue-scale co-association; Shapely edge correction down-weights discs that spill
outside the tissue window."

**T — Tool-capability matrix (stylised).**
File: `results/figures/14_T_tool_positioning.png`. Polished conference-style
capability table (capitalised headers, NoSegs row highlighted green, alternating
row shading, ✓/✗/– marks). Columns: Segmentation-free / Operates on raw
transcripts / Ligand–receptor co-localisation / Point-pattern statistics. Rows:
NoSegs (all four ✓), CellChat/LIANA, Squidpy gr.ripley, Bento, Baysor, FICTURE,
spatstat (R). NoSegs is the only all-✓ row.
*Caption.* "No published tool screens ligand–receptor co-localisation on raw
transcripts without segmentation. CellChat/LIANA, Squidpy, Bento and Baysor all
require segmentation or cell-typing; FICTURE is segmentation-free but answers a
different question (tissue domains); spatstat is a generic point-pattern library.
NoSegs satisfies all four capabilities."

**M6 — Workflow flowchart + quick-start guide. (REBUILT 2026-06-04 as native
PowerPoint shapes — supersedes the flat PNG.)**
**No longer a PNG.** Drawn directly in `docs/_build_poster.py` by the
`flowchart(x, y, w)` builder as **native, individually-editable autoshapes** (every
bubble's text/size/colour is editable in PowerPoint). Reason: Harry preferred the
original A1 poster's flowchart — punchier shape + colour — but wanted to keep the
A0 orange option-chips, and needed per-bubble editability that a flat matplotlib
PNG can't give. The nb14 `m6-flow-code` cell is kept as documentation but is
**superseded for the poster** by the native shapes.
Design (A1 punchy style + A0 chips, all native): vertical 8-stage `roundRect`
column with `downArrow` connectors — INPUT (green) → 1 PCA align + per-FOV GMM
strips → 2 DBSCAN noise removal → 3 observation window → **4 bivariate Ripley
K → L(r)** [teal] → **5 label-swap permutation envelope** [teal] → 6 effect-size
(SES) ranking → OUTPUTS networks·UMAP·heatmaps (blue). **Teal `#1A7A7A` highlights
the two statistical-core stages** (the novel method); purple `#500778` for the
prep/output stages. Amber option-chips (`#FFF4D6` fill, `#E0A800` edge) to the
right of the tunable steps: 3-component GMM · adaptive ε = p97 1-NN [20,30] px ·
rect/convex/concave window · multi-scale r-grid · n_sim 99/199 + edge correction ·
binary vs continuous SES. Light-purple quick-start panel at the foot with green
ticks: "use NoSegs when… segmentation fails / want L–R co-localisation from
transcripts / imaging platforms (CosMx, MERFISH, Xenium) / ≥ ~50 transcripts per
gene per ROI."
*Caption (legend in builder).* "The end-to-end workflow and a quick-start guide;
teal marks the statistical core, and amber chips mark the tunable option at each
step."

### Layout recommendation (A0 portrait, three columns) — REVISED 2026-06-04

> **BUILT 2026-06-04 evening → `docs/NoSegs_Poster_A0.pptx`** (true A0, native
> editable PowerPoint; original A1 `NoSegs_Poster.pptx` kept as fallback). The
> layout below is realised in that file with `[ FILL ]` guide prompts + per-figure
> legends. Regenerate with `docs/_build_poster.py`.
>
> **Finishing pass (2026-06-04, this session):** (1) M6 flowchart rebuilt as
> native editable shapes (see M6 entry — A1 punchy style + teal core + amber chips
> + quick-start); (2) **NoSegs** spelling fixed everywhere incl. both `.pptx`
> filenames (the 3 HPC `scripts/*.sh` keep "NoSeggs" in their `/scratch/...` paths
> intentionally — renaming would break CREATE re-runs); (3) **results take-home
> sharpened** — a purple banner over Fig F now leads with a bold one-line headline
> + a big **5/6** number callout + "+2.67 envelope half-widths" so the payoff lands
> fast (one `[ FILL: confirm headline wording ]` left for sign-off); (4) Negative-2
> control row dropped (above). Remaining: Harry fills the few `[ FILL ]` prompts
> + fact-checks, touches up spacing, then A4 test print → A0 order. See
> `notes/poster_writeup_guide.md` for the per-section reading list + key points.

Methodology-forward layout. A0 portrait ≈ 16× A4; room for the full set.

- **Left column (the problem + the wrangling):** Intro → **M0** raw-data layers
  (grounding view) + **M1** dropout under "The Problem" → **M2** GMM strips →
  **M3** DBSCAN QC.
- **Middle column (the method + that it works):** **M6** workflow flowchart +
  quick-start (the Methods visual anchor, replaces the old text pipeline) +
  effect-size pull-out → **M4** window iterations + **M5** r-scale/edge-correction
  + **T** positioning map + Fig B (control discrimination) + Validation table +
  Fig C (group SES).
- **Right column (the biology):** Fig F (infection — biggest results panel) +
  Fig G (network) + Fig H (UMAP).
- **Bottom strip:** Conclusions / references / acknowledgements / QR.

Notes:
- The M1–M5 suite + T map are individually polished standalone panels, so the
  PowerPoint layout is flexible — size/reorder freely. M1 is the grounding anchor.
- M1 *is* the dropout figure (formerly Fig A `01_vendor_segmentation_dropout.png`),
  rebuilt at poster grade — use M1, drop the old Fig A to avoid redundancy.
- Fig D (heatmap), Fig E (volcano), Fig I (diff SES) reserved for dissertation
  Results chapter — the methodology emphasis displaces them from the poster.
...
> - Fills a vacant niche: Bento, Squidpy, CellChat, LIANA all require segmentation. NoSegs does not.
